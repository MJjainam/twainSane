...
store_device(device_list_type device)
{
  ...
  {
    if (devices[i].method == device.method && !strcmp(devices[i].devname, device.devname) && devices[i].vendor == device.vendor && devices[i].product == device.product)
    {
    /*
  ...
  for (i = 0; i < ndev; i++)
  {
    ...
    for (interface = 0; (interface < config0->bNumInterfaces) && !found; interface++)
    {
      ...
      case LIBUSB_CLASS_PER_INTERFACE:
        if ((config0->interface[interface].num_altsetting == 0) || !config0->interface[interface].altsetting)
        {
          DBG(1, "%s: device 0x%04x/0x%04x doesn't "
                 "have an altsetting for interface %d\n",
              __func__,
      ...
      if (!found)
        DBG(5,
            ...
            (config0->interface[interface].num_altsetting != 0)
                ? config0->interface[interface].altsetting[0].bInterfaceClass
                : -1);
    }
                       ...
                       SANE_Status (*attach)(SANE_String_Const dev))
{
  ...
  {
    if (devices[dn].vendor == vendor && devices[dn].product == product && !devices[dn].missing && attach)
         ...
         pDescHead; pDescHead = GetNextDescriptor(pDescHead, pEndPtr))
    {
      ...
      case USB_DT_ENDPOINT:
        ...
        if (transfer_type == USB_ENDPOINT_TYPE_INTERRUPT)
        {
          ...
          else if (devices[devcount].int_out_ep)
        ...
        else
        {
          DBG(5, "sanei_usb_open: ignoring %s-%s endpoint "
                 "(address: %d)\n",
              transfer_type == USB_ENDPOINT_TYPE_CONTROL ? "control" : transfer_type == USB_ENDPOINT_TYPE_ISOCHRONOUS ? "isochronous" : "interrupt",
              direction ? "in" : "out", address);
          continue;
        }
        break;
...
void sanei_usb_set_timeout(SANE_Int __sane_unused__ timeout)
{
#if defined(HAVE_LIBUSB_LEGACY) || defined(HAVE_LIBUSB)
